...
#include "latch.h"
#include "delay.h"
code unsigned char tube_number_fixed[16]={0xC0 ,0xF9, 0xA4 ,0xB0 ,0x99 ,0x92 ,0x82,0xF8 ,0x80 ,0x90 ,0x88 ,0x83 ,0xC6 ,0xA1 ,0x86 ,0x8E};
void digitaltube_fixed(unsigned char tube_position,unsigned char tube_number)
{
	...
	latch(6);
	P0=tube_number_fixed[tube_number];
	latch(7);
	delay(2);
	P0=0xFF;
	latch(7);
...
void digitaltube_fixed_dp(unsigned char tube_position,unsigned char tube_number)
{
	switch(tube_position)
	{
		case 1: P0=0x01;break;
	...
	latch(6);
	P0=tube_number_fixed[tube_number]&0x7F;
	latch(7);
	delay(2);
	P0=0xFF;
	latch(7);
